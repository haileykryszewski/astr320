...
print("The PLX value represents the parallex to a given star.\n")
...
print('Question 6')
print('Assuming the same astronomer using the same telescope, the statsitical error will result from the a difference of the amount of measurements taken. For example, the fe_h list of values has 48 elements where as the PLX list only has 8. This is because fe_h has a broader a base of values to give a more accurate distribution. In order of statistical error from most statistically error prone to least statistically error prone is PM, PLX, oRV, Velocities, fe_h. The systematic error of the system is much easiser to predict since it is the same astronmer and the same instrument.')